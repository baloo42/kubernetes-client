...
public class CRDGenerator {
  ...
  public CRDGenerationInfo detailedGenerate() {
        ...
        if (parallel) {
          handlers.values().stream()
              .map(h -> ForkJoinPool.commonPool()
                  .submit(() -> h.handle(info, context.forkContext())))
              .forEach(ForkJoinTask::join);
        } else {
          handlers.values()
              .forEach(h -> h.handle(info, context.forkContext()));
        }
      }
    }
    ...
    handlers.values().stream()
        .flatMap(AbstractCustomResourceHandler::finish)
        .forEach(crdEntry -> emitCrd(enrichCrd(crdEntry.getKey()), crdEntry.getValue(), crdGenerationInfo));
  ...
  public void emitCrd(HasMetadata crd, Set<String> dependentClassNames,
